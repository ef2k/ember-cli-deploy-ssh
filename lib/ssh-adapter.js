...
var upload = function (indexBuffer) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (typeof _this.config.agent != 'undefined')
      ssh_config['agent'] = _this.config.agent;

    if (typeof _this.config.privateKeyFile != 'undefined')
      ssh_config['privateKey'] = require('fs').readFileSync(_this.config.privateKeyFile);

    if (typeof _this.config.passphrase != 'undefined')
      ssh_config['passphrase'] = _this.config.passphrase;
